@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex flex-col items-center">
        ...
        <div class="flex justify-center flex-1 sm:hidden w-full mb-2">
                ...
                <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            {{-- ... (kode untuk tombol panah kiri disabled) ... --}}
                    ...
                    {{-- Pagination Elements (Angka) --}}
                    @foreach ($elements as $element)
                        {{-- ... (kode untuk "Three Dots" Separator) ... --}}
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        {{-- Ganti warna pada span current page --}}
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white bg-[#1B3C53] border border-gray-300 cursor-default leading-5 dark:bg-gray-800 dark:border-gray-600">{{ $page }}</span>
                                    </span>
                                @else
                    ...
                    @else
                        <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                            {{-- ... (kode untuk tombol panah kanan disabled) ... --}}
